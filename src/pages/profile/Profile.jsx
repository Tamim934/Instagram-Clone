...
const Profile = () => {
  ...
  const [name, setName] = useState(
    localStorage.getItem("name") || "Cristiano_Ronaldo"
  );
  useEffect(() => {
  ...
  const handleSubmitName = () => {
    setIsLoading(true);
    axiosRequest
      .post("http://65.108.148.136:8085/User/get-users", { name: name })
      .then((response) => {
  ...
  const updateBio = () => {
    axiosRequest
      .post("http://65.108.148.136:8085/User/update-bio", { bio: userBio })
      .then((response) => {
